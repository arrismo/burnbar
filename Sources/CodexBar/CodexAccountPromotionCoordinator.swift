    ...
    static func mapUserFacingError(_ error: Error) -> CodexSystemAccountPromotionUserFacingError {
        ...
        if let error = error as? CodexAccountPromotionError {
            ...
            case .targetManagedAccountNotFound:
                "That account is no longer available in the app. Refresh the account list and try again."
            ...
            case .liveAccountUnreadable:
                "Could not read the current system account on this Mac."
            case .liveAccountMissingIdentityForPreservation:
                "Could not safely preserve the current system account before switching."
            case .liveAccountAPIKeyOnlyUnsupported:
                "Can't replace a system account that is signed in with an API key only setup."
            case .displacedLiveManagedAccountConflict:
                "Found another managed account that already uses the current system account. "
                    + "Resolve the duplicate account before switching."
            case .displacedLiveImportFailed:
                "Could not save the current system account before switching."
            case .managedStoreCommitFailed:
                "Could not update managed account storage."
            case .liveAuthSwapFailed:
                "Could not replace the live auth on this Mac."
            }
