...
read_when:
  - Using burnbar config from scripts or CI
...
`burnbar config` edits the same `~/.burnbar/config.json` file used by the app's Settings → Providers pane.
...
burnbar config providers --json --pretty
```
...
These are persistent app/CLI settings. They are different from `burnbar usage --provider grok`, which is a one-shot
command override and does not edit config.

If every provider is disabled, `burnbar usage` with no `--provider` prints no text output, and
`burnbar usage --json` prints `[]`. Passing `--provider <name>` still fetches that provider for the one command.
...
```bash
printf '%s' "$OPENROUTER_API_KEY" | burnbar config set-api-key --provider openrouter --stdin --no-enable
```
...
printf '%s' "$ANTHROPIC_ADMIN_KEY" | burnbar config set-api-key --provider claude --stdin
printf '%s' "$DEEPGRAM_API_KEY" | burnbar config set-api-key --provider deepgram --stdin
printf '%s' "$GROQ_API_KEY" | burnbar config set-api-key --provider groq --stdin
printf '%s' "$LLM_PROXY_API_KEY" | burnbar config set-api-key --provider llmproxy --stdin
printf '%s' "$Z_AI_API_KEY" | burnbar config set-api-key --provider zai --stdin
```
...
```bash
export BURNBAR_CONFIG=/tmp/burnbar-config.json
burnbar config enable --provider grok
burnbar config providers --json --pretty
```
...
```bash
burnbar cost --provider codex --days 90
burnbar cost --provider claude --days 180 --format json --pretty
```
